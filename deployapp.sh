# Work in progress to decouple the cloud stack deployment and EKS app deployment step
#!/usr/bin/env bash
set -ex
...
if sh -c "echo $USE_DATAGEN | grep -q -E '^(yes)$'" ; then
    ...
    if sh -c "echo $VARIANT | grep -q -E '^(Aerospike)$'" ; then
       ...
       make aerospike@datagen DATAGEN_CONCURRENCY=32 DATAGEN_ITEMS_PER_JOB=1000000  DATAGEN_DEVICES_ITEMS_PER_JOB=10000000 DATAGEN_DEVICES_PARALLELISM=100 STACK_NAME=${STACK_NAME} UNIQUEID=${UNID}
    else
